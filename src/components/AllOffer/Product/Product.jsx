...
const Product = ({ item }) => {
  ...
  const toggleCategory = (category) => {
    if (category === "Products Category") { // Change "Products Category" to match the name of your Product Category
      // Hide all categories when Product Category is clicked
      ...
      // Toggle visibility for other categories
      setVisibleCategories((prevState) => ({
        ...prevState,
  ...
  const handleSearch = (event) => {
    ...
    const filtered = Data.filter((item) =>
      item.head.toLowerCase().includes(term.toLowerCase())
    );
  ...
  return (
    <>
      <div className="main-products flex flex-wrap ml-10 h-[auto] mb-96">
        <div className="sub-products basis-1/4 max-w-72 mt-5 shadow-2xl p-4 max-md:hidden">
          ...
          <div className="products flex justify-between">
            ...
            <i onClick={togglePopupMenu}>
              {isPopupMenuOpen ? <HorizontalRuleIcon /> : <AddIcon />}
            </i>
          </div>
          {isPopupMenuOpen && (
          <div className="main-pro-list">
            ...
            <div className="other-pro-list mb-20">
              <div className="sub-pro-list text-xl font-semibold flex justify-between my-3 mx-3">
                <h1 onClick={() => toggleCategory("legs")}>Legs</h1>
                  <i onClick={() => toggleCategory("legs")}>
                    {visibleCategories.legs ? (<HorizontalRuleIcon />) : (<AddIcon />)}
                  </i>
              </div>
              {visibleCategories.legs && (
                  <>
                    <hr className="h-[3px] bg-[#898989]" />
                    <div className="category-data"> {/* Add your Legs data here */}</div>
                  </>
                )}
              <div className="sub-pro-list text-xl font-semibold flex justify-between my-3 mx-3">
                <h1 onClick={() => toggleCategory("color")}>Color</h1>
                  <i onClick={() => toggleCategory("color")}>
                    {visibleCategories.color ? (<HorizontalRuleIcon />) : (<AddIcon />)}
                  </i>
              </div>
              {visibleCategories.color && (
                  <>
                    <hr className="h-[3px] bg-[#898989]" />
                    <div className="category-data"> {/* Add your Color data here */}</div>
                  </>
                )}
              <div className="sub-pro-list text-xl font-semibold flex justify-between my-3 mx-3">
                <h1 onClick={() => toggleCategory("duration")} >Duration</h1>
                  <i onClick={() => toggleCategory("duration")}>
                    {visibleCategories.duration ? (<HorizontalRuleIcon />) : (<AddIcon />)}
                  </i>
              </div>
              {visibleCategories.duration && (
                  <>
                    <hr className="h-[3px] bg-[#898989]" />
                    <div className="category-data"> {/* Add your Duration data here */}</div>
                  </>
                )}
              <div className="sub-pro-list text-xl font-semibold flex justify-between my-3 mx-3">
                <h1 onClick={() => toggleCategory("price")} >Price</h1>
                  <i onClick={() => toggleCategory("price")}>
                    {visibleCategories.price ? (<HorizontalRuleIcon />) : (<AddIcon />)}
                  </i>
              </div>
              {visibleCategories.price && (
                  <>
                    <hr className="h-[3px] bg-[#898989]" />
                    <div className="category-data"> {/* Add your Price data here */}</div>
                  </>
                )}
            </div>
          </div>
        ...
        <div className="basis-3/4 w-[100%]">
          <div className="main-offer flex justify-between mt-5">
            <div className="flex">
              <h1 className="text-3xl font-bold ml-5 mt-1 max-md:text-2xl">
                All Offers
              </h1>
              <form className="exam mt-2 ml-10 max-md:hidden">
                <input
                  ...
                  className="w-[350px] sm:w[200px] rounded-md border border-gray-600 pl-6 py-1"
                />
              </form>
            </div>
            <div className="flex">
              ...
              <div className="main-box" key={index}>
                <div className="shadow-[0_3px_10px_rgb(0,0,0,0.2)] rounded-md py-4 pl-3">
                  ...
                  <div className="font-semibold">
                    <h4 className="text-[#DDDFE4] line-through">
                      {item.qar_1}
                    </h4>
                  ...
                  <div className="mt-2">
                    ...
                    <div className="flex justify-between">
                      ...
                      <div className="relative group">
                        <div
                          className="flex gap-2 p-2 border-2 w-[90px]  rounded-full hover:bg-teal-400 hover:shadow-lg"
                      ...
                      <div
                        className="flex gap-2 p-2 border-2 w-[90px]  rounded-full hover:bg-blue-500 hover:shadow-lg hover:text-white duration-300 ease-in-out"
                        onClick={() => handleClick("viewoffer", item, index)}
                      >
                        <i>
                          <item.ico_3 />
                        </i>
      ...
      {isShareMenuOpen && (
        <div className="share-menu">{/* Your share menu UI goes here */}</div>
      )}
    </>
  );
};
